...
interface ChatSession {
  id: string;
  title: string;
  updatedAt: any; // Firestore Timestamp — not string
}
...
export default function Sidebar({ 
  ...
  // Memoize filtered sessions to avoid recalculating on every render
  ...
  return (
    <>
      <AnimatePresence>
        {isOpen && (
          <motion.aside
            ...
            className="flex flex-col h-full bg-white border-r border-gray-200 overflow-hidden shrink-0 max-md:fixed max-md:z-40 max-md:h-full max-md:shadow-2xl"
          >
            {/* Header */}
      ...
      {isOpen && (
        <div 
          className="hidden max-md:block fixed inset-0 z-30 bg-black/20 backdrop-blur-sm"
          onClick={() => setIsOpen(false)}
        />
      )}
...
function SessionItem({ 
  ...
  // Fix: Sync local title state when parent updates (e.g. after AI auto-rename)
  useEffect(() => {
    setTitle(session.title);
  }, [session.title]);

  // Fix: Auto-dismiss delete confirmation after 5 seconds
  useEffect(() => {
    if (isDeleting) {
      const timer = setTimeout(() => setIsDeleting(false), 5000);
